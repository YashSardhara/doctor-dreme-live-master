...
function buildPrice(data) {
  ...
  if (boostPFSThemeConfig.custom.hasOwnProperty('show_price') &&
    ...
    if (onSale) {
      ...
      html += '<s>' + 'MRP:&nbsp;' + Utils.formatMoney(data.compare_at_price_min) + '</s> ';
      html += '<span class="boost-pfs-filter-product-item-sale-price plp_oriprice">' + Utils.formatMoney(data.price_min) + '</span>';
    } else {
      if (priceVaries) {
...
ProductSorting.prototype.compileTemplate = function() {
  ...
  if (boostPFSThemeConfig.custom.show_sort_by && boostPFSTemplate.hasOwnProperty('sortingHtml')) {
    ...
    var template = isMobile ?  boostPFSTemplate.sortingHtml :  boostPFSTemplate.sortingDesktopHtml;
    var activeClass = '';
    ...
    if (sortingArr) {
      ...
      for (var k in sortingArr) {
        ...
        if (isMobile) sortingItemsHtml += '<li aria-label="' + sortingArr[k] + '"><a href="#" data-sort="' + k + '" class="' + activeClass + '"  title="' + sortingArr[k] + '" aria-label="' + sortingArr[k] + '">' + sortingArr[k] + '</a></li>';
        else sortingItemsHtml += '<a href="#" data-sort="' + k + '" class="' + activeClass + '"  title="' + sortingArr[k] + '" aria-label="' + sortingArr[k] + '">' + sortingArr[k] + '</a>';
      }
      html = template.replace(/{{sortingItems}}/g, sortingItemsHtml);
    }
  }
  return html;
};
...
ProductSorting.prototype.render = function() {
  ...
  var labelSort = Labels.sorting_heading;
  /*
  var paramSort = Utils.stripHtml(Globals.queryParams.sort) || '';
...
ProductSorting.prototype.bindEvents = function() {
  var _this = this;
  jQ('.boost-pfs-filter-filter-dropdown a, .boost-pfs-sorting-container a').click(function(e) {
    e.preventDefault();
...
jQ(document).ready(function() {
  ...
  /* start-boost-custom */
  /* #boost-128853: fix price list items */
  var buildPriceListLabel = FilterOptionItem.prototype.buildPriceListLabel;
  FilterOptionItem.prototype.buildPriceListLabel = function() {
    var label = buildPriceListLabel.call(this);
    return label.replace(/&amp;#x20B9;/g, '₹');
  }
  
  ProductPaginationLoadMore.prototype.afterRender = function() {
    var title = jQ('.boost-pfs-filter-collection-header').text().replace(/\n/g,'').trim();
    var msg = Labels.loadMore.replace(/{{ amountProduct }}/g, title);
    
    this.$element.find('button').html(msg);
  }
  /* end-boost-custom */
})();